...
export class BackupManager {
   ...
   */
  public getBackupsRootPath(): string {
   ...
   */
  public async executeBackup(): Promise<string> {
    ...
    const storageSnapshot: BackupMetadata = {
      timestamp,
      version: '0.1.0-alpha',
      storagePolicy: {
   ...
   */
  public async restoreFromBackup(backupFolder: string): Promise<boolean> {
   ...
   */
  public createPreActionBackup(projectName: string, projectPath: string, commandPreview: string): string {
    ...
    this.fs.writeFileSync(
      this.path.join(targetDir, 'metadata.json'), 
