...
export class PluginManager {
  ...
  public log(pluginId: string, event: string, level: 'info' | 'warn' | 'error' = 'info') {
    this.logs.push({
      timestamp: Date.now(),
      plugin_id: pluginId,
      event,
  ...
  public getLogs(pluginId?: string): PluginLog[] {
    if (pluginId) {
  ...
  public runHealthCheck(pluginId: string): {
    healthy: boolean;
  ...
  public async executePluginTool(pluginId: string, toolName: string, executeFn: () => Promise<ToolResult>): Promise<ToolResult> {
    const p = this.plugins.get(pluginId);
    if (!p) {
  ...
  private async seoMetaAudit(url: string): Promise<ToolResult> {
    const auditReport = [
      `==========================================`,
  ...
  private async seoLinkCheck(url: string): Promise<ToolResult> {
    const checkReport = [
      `==========================================`,
  ...
  private async wpHeaderCheck(pluginPath: string): Promise<ToolResult> {
    const checkOutput = [
      `==========================================`,
  ...
  private async wpHooksAudit(pluginPath: string): Promise<ToolResult> {
    const checkOutput = [
      `==========================================`,
